...
public class Creador {
   ...
   public Cliente crearCliente(String datosUsuario[], String path){
        String crear = "INSERT INTO Cliente (codigo, nombre, DPI, pathDPI, direccion, sexo, password, birth, fechaIncorporacion, correo) VALUES(?,?,?,?,?,?,?,?,?,?)";
        ...
        try(PreparedStatement instrucciones = conexion.prepareStatement(crear)){            
            ...
            System.out.println(datosUsuario.length);
            ...
            instrucciones.setString(10, datosUsuario[5]);         
           
            
            instrucciones.executeUpdate();
            
            return conversor.convertirACliente(datosUsuario, codigo, herramientas.encriptarContrasenia(contrasenia), fechaActual, path);//recuerda que no usaste el returnGeneratedKeys por el hecho de que no es autoIncre... sino obliAutoIncre xD jajaja
   ...
   public Trabajador crearTrabajador(String tipoTrabajador, String[] datosTrabajador){
       String crear = "INSERT INTO "+ tipoTrabajador + "(codigo, nombre, DPI, direccion, sexo, password, turno, fechaIncorporacion, correo) VALUES (?,?,?,?,?,?,?,?,?)";
       ...
       try(PreparedStatement instrucciones = conexion.prepareStatement(crear)){
           ...
           instrucciones.setString(4, datosTrabajador[4]);
           instrucciones.setString(5, datosTrabajador[2]);
           instrucciones.setString(6, herramientas.encriptarContrasenia(contrasenia));
           instrucciones.setString(7, datosTrabajador[3]); 
           instrucciones.setString(8, fechaActual);         
           if(datosTrabajador.length>5){
               instrucciones.setString(9, datosTrabajador[5]);         
           }
           
           instrucciones.executeUpdate();
           
           return conversor.convertirATrabajador(tipoTrabajador, datosTrabajador, codigo, herramientas.encriptarContrasenia(contrasenia), fechaActual);
       }catch(SQLException sqlE){
   ...
   public Cuenta crearCuenta(String datosNuevaCuenta[]){
       String crear="INSERT INTO Cuenta (numeroCuenta, codigoDueno, fechaCreacion) VALUES(?,?,?)";
       ...
       try(PreparedStatement instrucciones = conexion.prepareStatement(crear)){                      
           ...
           instrucciones.setString(3, herramientas.darFechaActualString());
           
           instrucciones.executeUpdate();
           
           return conversor.convertirACuenta(elNumeroCuenta, dueno, 0, herramientas.darFechaActualString());//Esto por le momento, es solo para saber que si salió bien la creación puesto que para la "creacion de cuentas" no es necesario crear el obj... xD
